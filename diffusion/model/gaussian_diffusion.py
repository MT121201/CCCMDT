...
class LossType(enum.Enum):
    ...
    def is_vb(self):
        return self == LossType.KL or self == LossType
    

def _warmup_beta(beta_start, beta_end, num_diffusion_timesteps, warmup_frac):
    """
    Create a warmup schedule for beta values in a diffusion model.
    
    :param beta_start: Initial beta value at the start of the schedule.
    :param beta_end: Final beta value after warmup.
    :param num_diffusion_timesteps: Total number of timesteps in the diffusion process.
    :param warmup_frac: Fraction of timesteps used for linearly warming up beta values.
    :return: A numpy array of beta values with linear warmup followed by constant values.
    """
...
class GaussianDiffusion:
    ...
    def q_sample(self, x_start, t, noise=None):
        ...
        assert noise.shape == x_start.shape, "noise must have the same shape as x_start"
        return (
    ...
    def q_posterior_mean_variance(self, x_start, x_t, t):
        ...
        posterior_variance = _extract_into_tensor(self.posterior_variance, t, x_start.shape)
        posterior_log_variance_clipped = _extract_into_tensor(self.posterior_log_variance_clipped, t, x_start.shape)
    ...
    def p_mean_variance(self, model, x, t, clip_denoised=True, denoised_fn=None, model_kwargs=None):
        ...
        if model_kwargs is None:
            model_kwargs = {}
        B,C = x.shape[:2]
        assert t.shape[0] == B, "t must have the same batch size as x"
        ...
        if self.model_var_type in [ModelVarType.LEARNED, ModelVarType.LEARNED_RANGE]:
            ...
            # The model value is [-1,1] for [min_log, max_log]
            frac = (model_var_values + 1) / 2
            model_log_variance = frac * max_log + (1 - frac) * min_log
            model_variance = th.exp(model_log_variance)
        elif self.model_var_type in [ModelVarType.FIXED_SMALL, ModelVarType.FIXED_LARGE]:
        ...
        assert model_mean.shape == model_variance.shape == pred_xstart.shape == x.shape
        return {
            "mean": model_mean,
    ...
    def condition_mean(self, cond_fn, p_mean_var, x, t, model_kwargs=None):
        """
        Compute the mean for the previous timestep conditioned by a gradient function.
        
        :param cond_fn: Function that computes gradient of log p(y|x).
        :param p_mean_var: Dictionary with mean and variance from p_mean_variance.
        :param x: Current tensor.
        :param t: Current timestep.
        :param model_kwargs: Additional arguments for conditioning.
        :return: Adjusted mean for x_{t-1}.
        """
    ...
    def condition_score(self, cond_fn, p_mean_var, x, t, model_kwargs=None):
        """
        Adjust score-based model output based on a conditional function.
        
        :param cond_fn: Conditional gradient function.
        :param p_mean_var: Dictionary from p_mean_variance.
        :param x: Input tensor at x_t.
        :param t: Current timestep.
        :param model_kwargs: Additional arguments for conditioning.
        :return: Adjusted mean and prediction.
        """
    ...
    def p_sample(
        ...
        :param x: The tensor at timestep x_t.
        :param t: Current timestep.
        :param clip_denoised: Whether to clip the predicted x_0 to [-1, 1].
        :param denoised_fn: Optional function to adjust the x_0 prediction.
        :param cond_fn: Optional gradient function for conditional sampling.
        :param model_kwargs: Extra arguments for the model (for conditioning).
        :return: Dictionary with:
                - 'sample': Sample at x_{t-1}
                - 'pred_xstart': Prediction of x_0.
        """
        # Get model's mean and variance predictions
        out = self.p_mean_variance(
            model, x, t, clip_denoised=clip_denoised, denoised_fn=denoised_fn,
        ...
        # Adjust the mean if cond_fn is provided
    ...
    def p_sample_loop(self, model, shape, noise=None, clip_denoised=True, denoised_fn=None,
        ...
        """
        Generate final samples from the model.
        """
        final = None
    ...
    def p_sample_loop_progressive(self, model, shape, noise=None, clip_denoised=True, denoised_fn=None,
                                cond_fn=None, model_kwargs=None, device=None, progress=False):
        """
        Generates samples progressively and yields intermediate samples at each timestep.
        """
        # Set device
        ...
        # Loop through each timestep in reverse
        for i in indices:
            t = th.tensor([i] * shape[0], device=device)  # Set current timestep

            # Compute sample at current timestep
            with th.no_grad():
                out = self.p_sample(
                    model, img, t, clip_denoised=clip_denoised,
                    denoised_fn=denoised_fn, cond_fn=cond_fn, model_kwargs=model_kwargs
                )
                yield out  # Yield intermediate sample
                img = out["sample"]  # Update img for the next timestep
    ...
    def ddim_reverse_sample(
        ...
        """
        assert eta == 0.0, "Reverse ODE only for deterministic path (eta=0)"
        out = self.p_mean_variance(model, x, t, clip_denoised, denoised_fn, model_kwargs)
        if cond_fn is not None:
            out = self.condition_score(cond_fn, out, x, t, model_kwargs)
    ...
    def _vb_terms_bpd(
        ...
        """
        true_mean,_,true_log_variance = self.q_posterior_mean_variance(x_start=x_start, x_t=x_t, t=t)
        out = self.p_mean_variance(model, x_t, t, clip_denoised=clip_denoised, model_kwargs=model_kwargs)
        kl = normal_kl(true_mean, true_log_variance, out["mean"], out["log_variance"])
        kl = mean_flat(kl)/np.log(2)

        decoder_nll = -discretized_gaussian_log_likelihood(x_start, means=out["pred_xstart"], log_scales=out["log_variance"] * 0.5)
    ...
    def training_losses(self, model, x_start, timestep, model_kwargs=None, noise=None, skip_noise=False):
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            terms["loss"] = self._vb_terms_bpd(
                model, x_start, x_t, t, model_kwargs=model_kwargs
            )["output"]
            ...
            if self.snr:
                if self.model_mean_type == ModelMeanType.START_X:
                    ...
                    pred_noise = output
                    pred_xstart = self._predict_xstart_from_eps(x_t=x_t, t=t, eps=output)
                
                t = t[:, None, None, None].expand(pred_xstart.shape) # [128,4,32,32]
                #best
                target = th.where(t>249, noise, x_start)
                output = th.where(t>249, pred_noise, pred_xstart)
